...
public class PlayerInputManager : MonoBehaviour
{
    ...
    Vector2 _recentEndPressPosition;
    Vector2 _currentSwipe;
    float _buttonDownPhaseStart;
    public float tapInterval;
    public float swipeInterval = Screen.width / 7;
    ...
    void TouchInput()
    {
        foreach (Touch touch in Input.touches)
        {
            if (touch.phase == TouchPhase.Began)
            {
                ...
                _recentEndPressPosition = touch.position;
                _buttonDownPhaseStart = Time.time;
            }
            if (touch.phase == TouchPhase.Moved)
            {
                ...
                if ((touch.position - _recentEndPressPosition).x < -swipeInterval)
                {
                    Managers.Game.currentShape.movementController.MoveHorizontal(Vector2.left);
                    _recentEndPressPosition = touch.position;
                }
                else if ((touch.position - _recentEndPressPosition).x > swipeInterval)
                {
                    Managers.Game.currentShape.movementController.MoveHorizontal(Vector2.right);
                    _recentEndPressPosition = touch.position;
                }
            ...
            {
                Debug.Log(Time.time+ " / "+_buttonDownPhaseStart + "  /  " + tapInterval);
                if (Time.time - _buttonDownPhaseStart > tapInterval)
                {
                    //save ended touch 2d point
