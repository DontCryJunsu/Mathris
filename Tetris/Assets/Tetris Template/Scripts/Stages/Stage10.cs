...
public class Stage10 : MonoBehaviour
{
    ...
    void Operate()
    {
        ...
        int thisStageScore = (StageManager.Instance.numbers[0] * 10 + StageManager.Instance.numbers[1]) * (StageManager.Instance.numbers[2] *10 +StageManager.Instance.numbers[3]) / (StageManager.Instance.numbers[4]*10+ StageManager.Instance.numbers[5]) -  StageManager.Instance.numbers[6];
        int currentStageScore = PlayerPrefs.GetInt("Total")+thisStageScore;

        PlayerPrefs.SetInt("Total", currentStageScore);
        PlayerPrefs.SetInt("Stage10", thisStageScore);
