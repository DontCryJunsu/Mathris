...
public class Stage9 : MonoBehaviour
{
    ...
    void Operate()
    {
        ...
        int thisStageScore = (StageManager.Instance.numbers[0] * 10 + StageManager.Instance.numbers[1]) + StageManager.Instance.numbers[2] * 
            (StageManager.Instance.numbers[3]*10+ StageManager.Instance.numbers[4])-(StageManager.Instance.numbers[5]*10+ StageManager.Instance.numbers[6]);
        int currentStageScore = PlayerPrefs.GetInt("Total")+thisStageScore;

        PlayerPrefs.SetInt("Total", currentStageScore);
        PlayerPrefs.SetInt("Stage9", thisStageScore);
